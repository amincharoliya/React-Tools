...
const TodoApp = () => {
    ...
    const tasks = task.map(function(item,index){
        let Task = <li className="unchecked" key={index} onClick={e => {
    ...
    return(
        <HelmetProvider context = { helmetContext }>
            ...
            <div className="taskMain">
                ...
                <button className="dark-button" onClick={e => {todo !=='' ? setTask([...task,todo]) : alert('can\'t be blank');setTodo('')}} >Add</button>
                <ul className="tasks">
                    {tasks}                    
                </ul>
            </div>
